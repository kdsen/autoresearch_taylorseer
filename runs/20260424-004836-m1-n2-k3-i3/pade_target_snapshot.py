...
def pade_formula_mn(cache_dic: Dict, current: Dict) -> torch.Tensor:
    """
    Broad empirical monomial cubic family behind the historical Padé entrypoint.

    Replace the single-step Taylor polynomial with a coherence-shaped cubic in
    the ordinary monomial basis across all layers/modules. Residual compression
    remains broad and continuous rather than changing where the family runs.
    """
    x = current["step"] - current["activated_steps"][-1]
    factors = cache_dic["cache"][-1][current["layer"]][current["module"]]
    use_chebyshev_cubic = (
    ...
    quadratic_gain = 0.9375 + (0.0625 * coherence)
    cubic_gain = 0.5625 + (0.1875 * stability) + (0.125 * coherence)
    monomial_output = (
        factors[0]
        + linear
        + (quadratic_gain * quadratic)
        + (cubic_gain * cubic)
    )
    residual = monomial_output - taylor_output
