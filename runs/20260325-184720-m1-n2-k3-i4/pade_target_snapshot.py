...
def pade_coefficients_from_taylor(c, m, n, regularization=0.0):
    """
    ...
    if regularization > 0:
        ...
        solve_scale = A.abs().mean(dim=(-2, -1), keepdim=True) + 1.0
        A = A + regularization * solve_scale * eye
...
def _evaluate_generic_pade(factors, x, m, n, denom_threshold=1e-4) -> torch.Tensor:
    ...
    a, b = pade_coefficients_from_taylor(
        c,
        m,
        n,
        regularization=max(denom_threshold * 0.1, 1e-6),
    )
