...
def _evaluate_generic_pade(factors, x, m, n, denom_threshold=1e-4) -> torch.Tensor:
    ...
    lower_order_fallback = taylor_output
    if len(factors) >= 3 and (m, n) != (1, 1):
        lower_order_fallback = _evaluate_pade_11_from_factors(factors, x)
    ...
    weak_margin_mask = finite_mask & (denom_margin <= 1.5)
    candidate_output = torch.where(weak_margin_mask, lower_order_fallback, blended)
    return torch.where(finite_mask, candidate_output, lower_order_fallback)
