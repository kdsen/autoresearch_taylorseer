...
def pade_formula_mn(cache_dic: Dict, current: Dict) -> torch.Tensor:
    """
    Broad rational-Chebyshev cubic family behind the historical Padé entrypoint.

    Replace the single-step Taylor polynomial with a Chebyshev-basis cubic
    reconstruction across all layers/modules. A second-order rational
    denominator bounds the residual correction without shrinking where the
    family is applied.
    """
    x = current["step"] - current["activated_steps"][-1]
    factors = cache_dic["cache"][-1][current["layer"]][current["module"]]
    ...
    denom = 1.0 + (compression_strength * residual_ratio) + (
        0.5 * residual_ratio * residual_ratio
    )
    output = taylor_output + (residual / denom)
