...
def pade_formula_mn(cache_dic: Dict, current: Dict) -> torch.Tensor:
    """
    Conservative approximation-family hook behind the historical Padé entrypoint.

    This candidate keeps Taylor as the base path and only reweights the cubic
    term on single-step extrapolations. When the quadratic and cubic terms point
    in the same direction, or when the cubic correction is locally large, the
    hook decays toward a 2nd-order Taylor approximation instead of forcing a
    rational solve.
    ...
    taylor_2 = base + linear + quadratic

    base_scale = base.abs() + linear.abs() + quadratic.abs() + 1.0
    curvature_scale = base.abs() + linear.abs() + 1.0
    cubic_ratio = cubic.abs() / base_scale
    curvature_ratio = quadratic.abs() / curvature_scale

    amplification_risk = ((quadratic * cubic) > 0).to(taylor_output.dtype)
    damping = 1.0 / (1.0 + 2.5 * cubic_ratio + curvature_ratio)
    damping = damping * (1.0 - 0.35 * amplification_risk)
    damping = torch.clamp(damping, min=0.0, max=1.0)

    output = taylor_2 + damping * cubic
    finite_mask = torch.isfinite(output) & torch.isfinite(damping)
    output = torch.where(finite_mask, output, taylor_output)
