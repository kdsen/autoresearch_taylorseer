...
def pade_formula_mn(cache_dic: Dict, current: Dict) -> torch.Tensor:
    ...
    base_damped = taylor_output - ((1.0 - 0.623046875) * cubic)
    boosted_damped = taylor_output - ((1.0 - 0.62353515625) * cubic)
    safe_mask = torch.abs(quadratic) <= torch.abs(linear)
    damped = torch.where(safe_mask, boosted_damped, base_damped)
