...
def pade_formula_mn(cache_dic: Dict, current: Dict) -> torch.Tensor:
    ...
    linear = factors[1] * x
    cubic_retention = 0.623046875
    quadratic_retention = 0.99609375
    sign_mask = (quadratic * cubic) > 0
    sign_mask = sign_mask & ((linear * cubic) > 0)
    damped = taylor_output
    damped = damped - ((1.0 - cubic_retention) * cubic)
    damped = damped - ((1.0 - quadratic_retention) * quadratic)
