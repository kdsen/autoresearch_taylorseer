...
def pade_formula_mn(cache_dic: Dict, current: Dict) -> torch.Tensor:
    """
    Ultra-sparse empirical cubic family behind the historical Padé entrypoint.

    Keep the common path as close to pure Taylor as possible while testing a
    Hermite-style endpoint polynomial on the last-layer MLP single-step bucket.
    """
    x = current["step"] - current["activated_steps"][-1]
    factors = cache_dic["cache"][-1][current["layer"]][current["module"]]
    use_empirical_cubic = (
    ...
    curvature_lift = 1.001953125
    cubic_shape = 0.623046875
    hermite_output = factors[0] + linear + (curvature_lift * quadratic) + (cubic_shape * cubic)
    output = torch.where(sign_mask, hermite_output, taylor_output)
