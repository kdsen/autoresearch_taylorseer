...
def pade_formula_mn(cache_dic: Dict, current: Dict) -> torch.Tensor:
    ...
    reconstruction across all layers/modules. A continuous residual compressor
    keeps large local cubic corrections bounded without turning the candidate
    into a terminal-step or layer-only gate.
    """
    x = current["step"] - current["activated_steps"][-1]
    factors = cache_dic["cache"][-1][current["layer"]][current["module"]]
    use_chebyshev_cubic = (
