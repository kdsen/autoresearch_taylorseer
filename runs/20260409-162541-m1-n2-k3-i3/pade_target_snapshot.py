...
def pade_formula_mn(cache_dic: Dict, current: Dict) -> torch.Tensor:
    ...
    This candidate keeps Taylor as the base path and only soft-clips risky
    cubic extrapolation. When the quadratic and cubic terms point in the same
    direction and the cubic correction is locally large, the hook blends toward
    a 2nd-order Taylor approximation instead of forcing a rational solve.
    ...
    risk_scale = base.abs() + linear.abs() + quadratic.abs() + 1.0
    cubic_ratio = cubic.abs() / risk_scale
    ratio_threshold = 40.0 * cache_dic.get("pade_denom_threshold", 1e-4)
    amplifying = (quadratic * cubic) > 0
    excess_ratio = (cubic_ratio - ratio_threshold) / max(ratio_threshold, 1e-6)
    blend = torch.clamp(excess_ratio, min=0.0, max=1.0)
    blend = torch.where(amplifying, blend, torch.zeros_like(blend))

    output = blend * taylor_2 + (1.0 - blend) * taylor_output
    finite_mask = torch.isfinite(output) & torch.isfinite(blend)
