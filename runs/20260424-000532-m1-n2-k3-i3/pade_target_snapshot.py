...
def pade_formula_mn(cache_dic: Dict, current: Dict) -> torch.Tensor:
    """
    Broad Chebyshev cubic family behind the historical Padé entrypoint.

    Replace the single-step Taylor polynomial with a Chebyshev-basis cubic
    reconstruction across all layers/modules. A sign-aware residual compressor
    keeps anti-linear corrections tighter than aligned corrections without
    shrinking where the family is applied.
    """
    x = current["step"] - current["activated_steps"][-1]
    factors = cache_dic["cache"][-1][current["layer"]][current["module"]]
    use_chebyshev_cubic = (
    ...
    residual = chebyshev_output - taylor_output
    residual_ratio = torch.abs(residual) / local_scale
    residual_alignment = (residual * linear) / (
        (torch.abs(residual) * torch.abs(linear)) + 1e-12
    )
    residual_alignment = 0.5 + (0.5 * residual_alignment)
    compression_strength = 1.0 + (1.5 * (1.0 - coherence)) + (
        1.0 * (1.0 - residual_alignment)
    )
