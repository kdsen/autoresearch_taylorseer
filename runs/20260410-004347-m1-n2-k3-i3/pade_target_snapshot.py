...
def pade_formula_mn(cache_dic: Dict, current: Dict) -> torch.Tensor:
    ...
    confidence_mask = (quadratic * cubic) > 0
    confidence_mask = confidence_mask & (torch.abs(cubic) <= torch.abs(quadratic))
    output = torch.where(confidence_mask, damped, taylor_output)
