...
def pade_formula_mn(cache_dic: Dict, current: Dict) -> torch.Tensor:
    """
    Broad Hermite-style cubic family behind the historical Padé entrypoint.

    Replace the single-step Taylor polynomial with a Hermite-style cubic built
    from cached value/slope information and a sharpened coherence-shaped
    endpoint estimate across all layers/modules. Residual compression remains
    broad and continuous rather than changing where the family runs.
    """
    x = current["step"] - current["activated_steps"][-1]
    factors = cache_dic["cache"][-1][current["layer"]][current["module"]]
    use_chebyshev_cubic = (
    ...
    coherence_sharp = coherence * coherence
    endpoint_quadratic = 0.78125 + (0.125 * coherence) + (0.09375 * coherence_sharp)
    endpoint_cubic = 0.46875 + (0.15625 * stability) + (0.0625 * coherence) + (
        0.09375 * coherence_sharp
    )
    endpoint_value = factors[0] + linear + (endpoint_quadratic * quadratic) + (
        endpoint_cubic * cubic
    )
    slope_quadratic = 0.875 + (0.125 * coherence)
    slope_cubic = 0.5 + (0.125 * stability) + (0.0625 * coherence)
    endpoint_slope = (
        factors[1]
        + (slope_quadratic * factors[2] * x)
        + (0.5 * slope_cubic * factors[3] * (x ** 2))
    )

    # Map single-step offsets onto a unit Hermite segment: x=-1 -> u=1, x=1 -> u=0.
    u = (1.0 - x) * 0.5
    ...
    hermite_output = (
        ...
        + (h10 * span * factors[1])
        + (h01 * endpoint_value)
        + (h11 * span * endpoint_slope)
    )
    residual = hermite_output - taylor_output
    residual_ratio = torch.abs(residual) / local_scale
    compression_strength = 1.25 + (1.75 * (1.0 - coherence))
