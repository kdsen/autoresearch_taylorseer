...
def pade_formula_mn(cache_dic: Dict, current: Dict) -> torch.Tensor:
    """
    Ultra-sparse weighted-Taylor family behind the historical Padé entrypoint.

    Keep the common path as close to pure Taylor as possible: only the last-layer
    MLP in the very last denoising steps gets a conservative cubic damping.
    ...
    if not use_weighted_taylor:
        return taylor_output

    quadratic = 0.5 * factors[2] * (x ** 2)
    cubic = (factors[3] / 6.0) * (x ** 3)
    linear = factors[1] * x
    sign_mask = (quadratic * cubic) > 0
    sign_mask = sign_mask & ((linear * cubic) > 0)
    linear_abs = torch.abs(linear)
    quadratic_abs = torch.abs(quadratic)
    blend = torch.clamp(
        (linear_abs - quadratic_abs) / (linear_abs + 1e-12),
        min=0.0,
        max=1.0,
    )
    cubic_retention = 0.623046875 + ((0.62353515625 - 0.623046875) * blend)
    damped = taylor_output - ((1.0 - cubic_retention) * cubic)
    output = torch.where(sign_mask, damped, taylor_output)
