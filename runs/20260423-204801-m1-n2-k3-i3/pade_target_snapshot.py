...
def pade_formula_mn(cache_dic: Dict, current: Dict) -> torch.Tensor:
    """
    Ultra-sparse Legendre cubic family behind the historical Padé entrypoint.

    Keep the common path as close to pure Taylor as possible while testing a
    tiny orthogonal-basis curvature contraction on the last-layer MLP
    single-step bucket.
    """
    x = current["step"] - current["activated_steps"][-1]
    factors = cache_dic["cache"][-1][current["layer"]][current["module"]]
    ...
    p1 = x
    p2 = 0.5 * ((3.0 * (x ** 2)) - 1.0)
    p3 = 0.5 * ((5.0 * (x ** 3)) - (3.0 * x))
    p2_gain = 0.998046875
    cubic_shape = 0.623046875
    legendre_output = (
        factors[0]
        + (q_coeff / 3.0)
        + ((factors[1] + ((3.0 * cubic_shape * r_coeff) / 5.0)) * p1)
        + (p2_gain * ((2.0 * q_coeff) / 3.0) * p2)
        + (((2.0 * cubic_shape * r_coeff) / 5.0) * p3)
    )
    output = torch.where(sign_mask, legendre_output, taylor_output)
