...
def pade_coefficients_from_taylor(c, m, n):
    ...
    # Padé coefficients are invariant to a shared scale factor on c_k. Keep the
    # solve in a better numeric range, but avoid upscaling tiny coefficients.
    coeff_scale = torch.ones_like(c[0].abs())
    for coeff in c[: m + n + 1]:
        coeff_scale = torch.maximum(coeff_scale, coeff.abs())
    scaled_c = [coeff / coeff_scale for coeff in c]

    rows = []
    rhs_terms = []
    for row in range(n):
        k = m + 1 + row
        rhs_terms.append(-scaled_c[k])
        rows.append(torch.stack([scaled_c[k - col - 1] for col in range(n)], dim=-1))
    ...
    for i in range(m + 1):
        ...
        for j in range(1, min(i, n) + 1):
            s += b[..., j - 1] * scaled_c[i - j]
        a.append(s * coeff_scale)
...
def _evaluate_generic_pade(factors, x, m, n, denom_threshold=1e-4) -> torch.Tensor:
    ...
    safe_mask = safe_mask & (pole_risk < 0.5)

    blend = torch.minimum(denom_blend, pole_blend)
