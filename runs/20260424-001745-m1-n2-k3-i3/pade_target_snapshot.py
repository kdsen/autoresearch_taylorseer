...
def pade_formula_mn(cache_dic: Dict, current: Dict) -> torch.Tensor:
    ...
    reconstruction across all layers/modules. A sharpened coherence response
    lets strongly aligned regions keep a more assertive cubic shape while
    incoherent regions remain more heavily compressed.
    """
    x = current["step"] - current["activated_steps"][-1]
    factors = cache_dic["cache"][-1][current["layer"]][current["module"]]
    use_chebyshev_cubic = (
    ...
    t2_gain = 1.0 + (0.000244140625 * stability * (0.25 + (0.75 * coherence_sharp)))
    cubic_shape = 0.625 + (0.15625 * stability) + (0.21875 * coherence_sharp)
    chebyshev_output = (
        factors[0]
        + (q_coeff / 2.0)
    ...
    compression_strength = 1.1 + (1.9 * (1.0 - coherence_sharp))
