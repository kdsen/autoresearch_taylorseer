...
def pade_formula_mn(cache_dic: Dict, current: Dict) -> torch.Tensor:
    """
    Ultra-sparse monomial cubic family behind the historical Padé entrypoint.

    Keep the common path as close to pure Taylor as possible while testing a
    tiny empirical coefficient contraction on the last-layer MLP single-step
    bucket.
    """
    x = current["step"] - current["activated_steps"][-1]
    factors = cache_dic["cache"][-1][current["layer"]][current["module"]]
    ...
    linear_gain = 0.9999847412109375
    cubic_shape = 0.623046875
    monomial_output = factors[0] + (linear_gain * linear) + quadratic + (cubic_shape * cubic)
    output = torch.where(sign_mask, monomial_output, taylor_output)
