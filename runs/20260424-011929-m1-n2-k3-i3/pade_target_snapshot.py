...
def pade_formula_mn(cache_dic: Dict, current: Dict) -> torch.Tensor:
    ...
    from cached value/slope information and a coherence-shaped endpoint
    estimate across all layers/modules. Residual compression remains broad and
    continuous rather than changing where the family runs.
    """
    x = current["step"] - current["activated_steps"][-1]
    factors = cache_dic["cache"][-1][current["layer"]][current["module"]]
    use_chebyshev_cubic = (
    ...
    endpoint_quadratic = 0.75 + (0.25 * coherence)
    endpoint_cubic = 0.5 + (0.125 * stability) + (0.125 * coherence)
