...
def pade_formula_mn(cache_dic: Dict, current: Dict) -> torch.Tensor:
    """
    Ultra-sparse Chebyshev cubic family behind the historical Padé entrypoint.

    Keep the common path as close to pure Taylor as possible while testing a
    tiny Chebyshev-basis curvature expansion on the last-layer MLP single-step
    bucket.
    """
    x = current["step"] - current["activated_steps"][-1]
    factors = cache_dic["cache"][-1][current["layer"]][current["module"]]
    use_empirical_cubic = (
        ...
        and len(factors) >= 4
        and current.get("module") == "mlp"
        and current.get("layer") == (len(cache_dic["cache"][-1]) - 1)
        and current["step"] == 1
    )
    ...
    cubic = r_coeff * (x ** 3)
    linear = factors[1] * x
    sign_mask = (quadratic * cubic) > 0
    sign_mask = sign_mask & ((linear * cubic) > 0)
    envelope = torch.abs(linear) + torch.abs(quadratic) + 1e-12
    sign_mask = sign_mask & (torch.abs(cubic) <= (0.75 * envelope))

    t1 = x
    t2 = (2.0 * (x ** 2)) - 1.0
    t3 = (4.0 * (x ** 3)) - (3.0 * x)
    t2_gain = 1.000244140625
    cubic_shape = 0.623046875
    chebyshev_output = (
        factors[0]
        + (q_coeff / 2.0)
    ...
    output = torch.where(sign_mask, chebyshev_output, taylor_output)
