...
def summarize_approx_stats(cache_dic: Dict) -> Dict:
    ...
    return {
        ...
        "pade_steps": int(stats.get("pade_steps", 0)),
        "taylor_steps": int(stats.get("taylor_steps", 0)),
        "mixed_steps": int(stats.get("mixed_steps", 0)),
        "pure_pade_steps": int(stats.get("pure_pade_steps", 0)),
        "pure_taylor_steps": int(stats.get("pure_taylor_steps", 0)),
        "pade_calls": int(pade_calls),
        "taylor_calls": int(taylor_calls),
        "pade_step_ratio": (
            float(stats["pade_steps"] / stats["total_steps"])
            if stats.get("total_steps", 0)
            else 0.0
        ),
...
def _record_approx_mode(cache_dic: Dict, current: Dict, mode: str) -> None:
    ...
    mode_bit = 1 if mode == "pade" else 2
    prev_mask = stats["step_masks"].get(step, 0)
    new_mask = prev_mask | mode_bit
    ...
    if new_mask != prev_mask:
        ...
        if mode == "pade":
            ...
            stats["taylor_steps"] += 1

        if new_mask == 1:
            stats["pure_pade_steps"] += 1
        elif new_mask == 2:
            stats["pure_taylor_steps"] += 1
        elif new_mask == 3:
            stats["mixed_steps"] += 1
            if prev_mask == 1:
                stats["pure_pade_steps"] -= 1
        ...
        stats["step_masks"][step] = new_mask
...
def _evaluate_generic_pade(factors, x, m, n, denom_threshold=1e-4) -> torch.Tensor:
    ...
    for j in range(1, n + 1):
        ...
        denom_scale = denom_scale + term.abs()

    solve_residual = torch.matmul(A, b.unsqueeze(-1)).squeeze(-1) - rhs
    solve_scale = (
        torch.linalg.vector_norm(rhs, dim=-1)
        + torch.linalg.matrix_norm(A, dim=(-2, -1)) * torch.linalg.vector_norm(b, dim=-1)
        + 1.0
    )
    residual_ok = torch.linalg.vector_norm(solve_residual, dim=-1) <= (1e-3 * solve_scale)
    ...
    blend = torch.clamp(denom_margin - 1.0, min=0.0, max=1.0)
    denom_departure = (denominator - 1.0).abs() / denom_scale.clamp_min(1.0)
    correction_scale = taylor_output.abs() + pade_value.abs() + 1.0
    correction_ratio = (pade_value - taylor_output).abs() / correction_scale
    denominator_guard = 1.0 / (1.0 + 2.0 * denom_departure)
    correction_guard = 1.0 / (1.0 + 4.0 * correction_ratio)
    blend = blend * denominator_guard * correction_guard
    blended = blend * pade_value + (1.0 - blend) * taylor_output
    return torch.where(finite_mask, blended, taylor_output)


def pade_formula_mn(cache_dic: Dict, current: Dict) -> torch.Tensor:
    """
    Broad piecewise cubic family behind the historical Padé entrypoint.

    Replace the single-step Taylor polynomial with a Chebyshev-basis cubic
    reconstruction across all layers/modules. Local derivative coherence now
    chooses between a stronger Chebyshev law and a conservative monomial cubic
    law before a continuous residual compressor bounds large corrections.
    """
    x = current["step"] - current["activated_steps"][-1]
    factors = cache_dic["cache"][-1][current["layer"]][current["module"]]
    use_chebyshev_cubic = (
        cache_dic.get("enable_pade", False)
        and (not cache_dic.get("pade_only_single_step", True) or abs(x) == 1)
        and len(factors) >= 4
    )

    taylor_output = 0
    for i in range(len(factors)):
        taylor_output += (1 / math.factorial(i)) * factors[i] * (x ** i)

    if not use_chebyshev_cubic:
        return taylor_output

    q_coeff = 0.5 * factors[2]
    r_coeff = factors[3] / 6.0
    linear = factors[1] * x
    quadratic = q_coeff * (x ** 2)
    cubic = r_coeff * (x ** 3)

    t1 = x
    t2 = (2.0 * (x ** 2)) - 1.0
    t3 = (4.0 * (x ** 3)) - (3.0 * x)
    local_scale = torch.abs(linear) + torch.abs(quadratic) + torch.abs(cubic) + 1e-12
    cubic_share = torch.abs(cubic) / local_scale
    stability = 1.0 - torch.clamp(cubic_share, min=0.0, max=1.0)
    linear_alignment = (linear * cubic) / ((torch.abs(linear) * torch.abs(cubic)) + 1e-12)
    quadratic_alignment = (quadratic * cubic) / (
        (torch.abs(quadratic) * torch.abs(cubic)) + 1e-12
    )
    coherence = 0.5 + (0.25 * (linear_alignment + quadratic_alignment))
    coherence = torch.clamp(coherence, min=0.0, max=1.0)
    t2_gain = 1.0 + (0.000244140625 * stability * (0.5 + (0.5 * coherence)))
    cubic_shape = 0.625 + (0.1875 * stability) + (0.1875 * coherence)
    chebyshev_output = (
        factors[0]
        + (q_coeff / 2.0)
        + ((factors[1] + ((3.0 * cubic_shape * r_coeff) / 4.0)) * t1)
        + (t2_gain * (q_coeff / 2.0) * t2)
        + ((cubic_shape * r_coeff / 4.0) * t3)
    )
    conservative_quadratic = 0.8125 + (0.1875 * stability)
    conservative_cubic = 0.3125 + (0.1875 * stability) + (0.125 * coherence)
    monomial_output = (
        factors[0]
        + linear
        + (conservative_quadratic * quadratic)
        + (conservative_cubic * cubic)
    )
    regime_blend = torch.clamp((1.5 * coherence) - 0.25, min=0.0, max=1.0)
    piecewise_output = (regime_blend * chebyshev_output) + (
        (1.0 - regime_blend) * monomial_output
    )
    residual = piecewise_output - taylor_output
    residual_ratio = torch.abs(residual) / local_scale
    regime_gap = torch.abs(chebyshev_output - monomial_output) / local_scale
    compression_strength = (
        1.25 + (1.75 * (1.0 - coherence)) + (0.5 * torch.clamp(regime_gap, max=1.0))
    )
    residual_weight = 1.0 / (1.0 + (compression_strength * residual_ratio))
    output = taylor_output + (residual_weight * residual)
