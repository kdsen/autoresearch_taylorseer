...
def pade_formula_mn(cache_dic: Dict, current: Dict) -> torch.Tensor:
    ...
    This candidate keeps Taylor as the base path and only clips risky cubic
    extrapolation. When the quadratic and cubic terms point in the same
    direction and the cubic correction is locally large, the hook falls back to
    ...
    unsafe_cubic = ((quadratic * cubic) > 0) & (cubic_ratio > ratio_threshold)

    output = torch.where(unsafe_cubic, taylor_2, taylor_output)
