...
def pade_formula_mn(cache_dic: Dict, current: Dict) -> torch.Tensor:
    """
    Broad orthogonal-basis cubic family behind the historical Padé entrypoint.
    ...
    blends Chebyshev and Legendre-style cubic laws before a continuous residual
    compressor bounds large corrections.
    """
    x = current["step"] - current["activated_steps"][-1]
    factors = cache_dic["cache"][-1][current["layer"]][current["module"]]
    use_chebyshev_cubic = (
    ...
    p2 = (1.5 * (x ** 2)) - 0.5
    p3 = (2.5 * (x ** 3)) - (1.5 * x)
    p2_gain = 0.875 + (0.125 * stability)
    p3_shape = 0.5 + (0.125 * stability) + (0.125 * coherence)
    legendre_output = (
        factors[0]
        + (q_coeff / 3.0)
        + ((factors[1] + ((3.0 * p3_shape * r_coeff) / 5.0)) * p1)
        + (p2_gain * ((2.0 * q_coeff) / 3.0) * p2)
        + (p3_shape * ((2.0 * r_coeff) / 5.0) * p3)
    )
    basis_blend = 0.25 + (0.75 * coherence)
    orthogonal_output = (basis_blend * chebyshev_output) + (
        (1.0 - basis_blend) * legendre_output
    )
    residual = orthogonal_output - taylor_output
    residual_ratio = torch.abs(residual) / local_scale
    basis_gap = torch.abs(chebyshev_output - legendre_output) / local_scale
    compression_strength = (
        1.0 + (1.25 * (1.0 - coherence)) + (0.75 * torch.clamp(basis_gap, max=1.0))
    )
